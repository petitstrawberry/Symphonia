...
impl FormatReader for AdtsReader<'_> {
    ...
    fn next_packet(&mut self) -> Result<Option<Packet>> {
        ...
        Ok(Some(Packet::new(
            ...
            self.reader
                .read_boxed_slice_exact(usize::from(header.payload_len()))?,
        )))
    }
    ...
    fn seek(&mut self, _mode: SeekMode, to: SeekTo) -> Result<SeekedTo> {
        ...
        loop {
            ...
            let next_packet_ts = match self.next_packet_ts.checked_add(SAMPLES_PER_AAC_PACKET) {
                ...
                _ => {
                    self.reader
                        .seek_buffered_rev(usize::from(header.header_len()));
                    break;
                }
            };
...
fn approximate_frame_count(mut source: &mut MediaSourceStream<'_>) -> Result<Option<u64>> {
    ...
    if !source.is_seekable() {
        ...
        while let Ok(header) = AdtsHeader::read(&mut scoped_stream) {
            if scoped_stream
                .ignore_bytes(u64::from(header.payload_len()))
                .is_err()
            {
                break;
            }
    ...
    match parsed_n_frames {
        0 => Ok(None),
        _ => Ok(Some(
            remaining_len / (n_bytes / parsed_n_frames) * SAMPLES_PER_AAC_PACKET.get(),
        )),
    }
}
